...
var CFG = {
  ...
  exportFileName: 'AGBD_Prediction',
...
function visualizeResults(prediction, region, year) {
  ...
  Map.addLayer(prediction, agbdViz, 'AGBD Prediksi ' + year + ' (Mg/ha)');

  var legend = ui.Panel({
    style: { position: 'bottom-left', padding: '8px 12px', backgroundColor: 'rgba(255,255,255,0.9)' }
  });
  legend.add(ui.Label({
    value: 'AGBD Predicted (Mg/ha)',
  ...
  ['#ffffcc', '#c7e9b4', '#7fcdbb', '#41b6c4', '#2c7fb8', '#253494'].forEach(function (col, i) {
    legend.add(ui.Panel([
      ...
      ui.Label(['0', '60', '120', '180', '240', '300'][i] + ' Mg/ha', { fontSize: '11px', margin: '2px 0' })
